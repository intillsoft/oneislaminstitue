...
export const TextBlock = ({ content }) => (
  <div className="prose prose-slate dark:prose-invert max-w-none">
    <div className="text-sm text-[var(--color-text-secondary)] dark:text-slate-400 leading-relaxed whitespace-pre-wrap font-medium">
      {content.text}
    </div>
  </div>
);
...
export const ScriptureBlock = ({ content }) => (
  <div className="my-8 p-6 sm:p-8 bg-[var(--color-primary-light)] dark:bg-emerald-950/30 rounded-2xl sm:rounded-3xl border border-[var(--color-border-secondary)] dark:border-emerald-900/50 relative overflow-hidden group">
    <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform">
      <Icon name="BookOpen" size={48} className="text-[var(--color-primary)]" />
    </div>
    <div className="relative z-10 space-y-6">
      ...
      <p className="text-xs sm:text-sm font-bold italic text-slate-700 dark:text-slate-300 border-l-2 border-[var(--color-primary)]/30 pl-4 sm:pl-6 leading-relaxed">
        "{content.translation}"
      </p>
      <div className="flex justify-end pt-2">
        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-primary)]/60 bg-[var(--color-primary)]/5 px-3 py-1 rounded-full border border-emerald-600/10">
          — {content.reference}
        </span>
      </div>
    </div>
    ...
    <div className="my-10 perspective-1000 group cursor-pointer" onClick={() => setIsFlipped(!isFlipped)}>
      ...
      >
        ...
        <div className={`absolute inset-0 backface-hidden rounded-[1.5rem] sm:rounded-[2.5rem] bg-white dark:bg-[#13182E] border border-slate-100 dark:border-white/5 shadow-premium p-6 sm:p-10 flex flex-col justify-between overflow-hidden`}>
           <div className="absolute top-0 right-0 w-32 h-32 bg-[var(--color-primary)]/5 rounded-full blur-3xl -mr-16 -mt-16" />
           <div className="space-y-6 relative z-10">
              <div className="flex items-center gap-3">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--color-primary)]">Prophetic Guidance</span>
              </div>
           ...
           <div className="flex items-center justify-between mt-6 relative z-10">
              ...
              <span className="text-[10px] font-black text-[var(--color-primary)]/60 uppercase tracking-widest">— {content.narrator || 'Authentic Hadith'}</span>
           </div>
        </div>

        {/* Back Side (Arabic Context) */}
        <div className={`absolute inset-0 backface-hidden rounded-[1.5rem] sm:rounded-[2.5rem] bg-[var(--color-primary)] p-6 sm:p-10 flex flex-col justify-center items-center text-center shadow-glow overflow-hidden`} style={{ transform: 'rotateY(180deg)' }}>
...
export const QuranVerseCard = ({ content }) => (
  <div className="my-10 relative group">
    <div className="absolute -inset-1 bg-gradient-to-r from-[var(--color-primary)] to-teal-500 rounded-[2rem] sm:rounded-[3rem] blur opacity-15 group-hover:opacity-25 transition duration-1000 group-hover:duration-200" />
    <div className="relative bg-white dark:bg-[var(--color-bg-dark)] rounded-[1.5rem] sm:rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-2xl overflow-hidden p-6 sm:p-12 space-y-6 sm:space-y-8">
      <div className="flex items-center justify-between">
         <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-[1.2rem] bg-[var(--color-primary)] flex items-center justify-center text-white shadow-lg shadow-[var(--color-primary)]/20">
               <Icon name="Book" size={24} />
            </div>
            <div>
               <h4 className="text-[11px] font-black text-text-primary uppercase tracking-[0.3em]">Noble Quran</h4>
               <p className="text-[9px] font-bold text-[var(--color-primary)] uppercase tracking-widest mt-0.5">{content.surah}</p>
            </div>
         </div>
         <div className="w-10 h-10 rounded-full bg-white dark:bg-white/5 border border-slate-100 dark:border-white/10 flex items-center justify-center text-slate-400">
      ...
      <div className="space-y-8">
        ...
        <div className="p-6 sm:p-8 bg-[var(--color-primary-light)]/50 dark:bg-[var(--color-primary)]/5 rounded-2xl sm:rounded-3xl border border-[var(--color-border-secondary)] dark:border-[var(--color-primary)]/10">
      ...
      <div className="pt-2 flex justify-center">
         <div className="h-0.5 w-16 bg-[var(--color-primary)]/20 rounded-full" />
      </div>
    </div>
  </div>
);
...
export const DesignMarkdownBlock = ({ content }) => (
  <div className="my-10 p-1 bg-gradient-to-br from-[var(--color-primary)]/20 via-slate-200/50 to-[var(--color-primary)]/10 dark:from-[var(--color-primary)]/10 dark:via-white/5 dark:to-[var(--color-primary)]/5 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-premium overflow-hidden">
    <div className="bg-white dark:bg-[#13182E] rounded-[1.4rem] sm:rounded-[2.4rem] p-6 sm:p-10 space-y-6">
       {content.title && (
         <div className="flex items-center gap-3 mb-6">
            <div className="w-2 h-6 bg-[var(--color-primary)] rounded-full shadow-glow" />
            <h3 className="text-[11px] font-black text-text-primary uppercase tracking-[0.4em]">{content.title}</h3>
         </div>
       )}
       <div className="prose prose-slate dark:prose-invert max-w-none">
          <div className="text-xs sm:text-sm text-[var(--color-text-secondary)] dark:text-slate-300 leading-[1.8] font-medium space-y-4">
             {content.text.split('\n\n').map((p, i) => (
               <p key={i}>{p}</p>
             ))}
          </div>
...
export const MediaBlock = ({ content }) => {
  ...
  return (
    <div className="my-8 rounded-3xl overflow-hidden border border-[var(--color-border-primary)] dark:border-white/5 shadow-lg bg-black">
      {embedUrl ? (
        <div className="relative w-full aspect-video">
          ...
          <div className="absolute bottom-4 right-4 z-50">
               <a 
                 ...
                 className="flex items-center gap-1.5 px-3 py-2 bg-black/60 hover:bg-black/80 backdrop-blur-md border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-wider text-white hover:text-[var(--color-primary)] transition-all shadow-xl"
               >
                    <Icon name="ExternalLink" size={11} className="text-[var(--color-primary)]" /> Watch on YouTube
               </a>
          </div>
        </div>
      ) : (
        <div className="p-12 text-center text-white flex flex-col items-center justify-center space-y-4">
           <Icon name="Video" size={40} className="text-[var(--color-primary)]" />
           <p className="text-sm font-bold uppercase tracking-widest text-slate-400">External Media Stream</p>
           <a href={content.url} target="_blank" rel="noopener noreferrer" className="px-6 py-2 bg-[var(--color-primary)] rounded-full text-[10px] font-bold uppercase tracking-widest hover:scale-105 transition-all">Launch Player</a>
        </div>
      )}
    </div>
  );
...
export const AudioBlock = ({ content }) => (
  <div className="my-6 p-4 sm:p-6 bg-white dark:bg-white/[0.02] rounded-2xl border border-slate-100 dark:border-white/5 flex items-center gap-4 sm:gap-6 group hover:bg-white dark:hover:bg-white/5 transition-all shadow-soft active:scale-[0.98]">
    <button className="w-14 h-14 bg-[var(--color-primary)] text-white rounded-2xl flex items-center justify-center shadow-lg shadow-[var(--color-primary)]/20 group-hover:scale-110 transition-transform">
      <Icon name="Play" size={24} fill="currentColor" />
    </button>
    <div className="flex-1 min-w-0">
      <p className="text-xs font-bold text-[var(--color-text-primary)] dark:text-white truncate uppercase tracking-widest mb-1">{content.title || 'Audio Lecture'}</p>
    ...
    <span className="text-[10px] font-bold text-[var(--color-text-tertiary)] uppercase tracking-widest">{content.duration || '00:00'}</span>
  </div>
);
...
export const FileBlock = ({ content }) => (
  <div className="my-4 p-4 bg-white dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/10 flex items-center justify-between group hover:border-emerald-600/30 transition-all">
    ...
    <button className="p-2 text-slate-400 hover:text-[var(--color-primary)] transition-colors">
...
export const SummaryBlock = ({ content }) => (
  <div className="my-10 p-6 sm:p-10 bg-gradient-to-br from-slate-900 to-slate-800 dark:from-black dark:to-slate-900 rounded-2xl sm:rounded-3xl border border-white/10 shadow-xl relative overflow-hidden text-white group">
    ...
    <div className="relative z-10">
      <h3 className="text-[10px] font-bold text-[var(--color-primary)] uppercase tracking-[0.3em] mb-6 flex items-center gap-2">
      ...
      <ul className="space-y-4">
        {content.points.map((point, i) => (
          <li key={i} className="flex items-start gap-4">
            <div className="w-5 h-5 rounded-full bg-[var(--color-primary)]/20 border border-[var(--color-primary)]/40 flex items-center justify-center shrink-0 mt-0.5">
               <Icon name="Check" size={10} className="text-[var(--color-primary)]" />
            </div>
...
export const ImageBlock = ({ content }) => (
  <div className="my-8 rounded-xl sm:rounded-[2rem] overflow-hidden border border-[var(--color-border-primary)] dark:border-white/5 shadow-premium group">
    <img 
      src={content.url} 
    ...
    />
    {content.caption && (
      <div className="p-4 bg-white dark:bg-white/5 border-t border-slate-100 dark:border-white/5">
