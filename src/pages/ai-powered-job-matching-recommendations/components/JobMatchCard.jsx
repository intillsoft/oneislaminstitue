...
const JobMatchCard = ({ job, userSkills, onSelect, isSelected, onExplain }) => {
  ...
  const getMatchColor = (score) => {
    if (score >= 90) return 'text-[var(--color-primary)] bg-[var(--color-primary)]/10 border-[var(--color-primary)]/20';
    ...
    return 'text-slate-400 bg-slate-100 dark:bg-white/5 border-[var(--color-border-primary)] dark:border-white/10';
  };

  const getTrendIcon = (trend) => {
    switch (trend?.toLowerCase()) {
      case 'very high': case 'high':
        return <Icon name="TrendingUp" size={14} className="text-[var(--color-primary)]" />;
      case 'rising':
  ...
  return (
    <div
      className={`glass-panel group relative overflow-hidden rounded-[2rem] border-2 transition-all duration-300 cursor-pointer ${isSelected
        ...
        : 'border-[var(--color-border-primary)] dark:border-white/5 bg-white/50 dark:bg-white/5 hover:border-workflow-primary/30 hover:shadow-2xl'
        }`}
    ...
    >
      ...
      <div className="relative w-full h-40 overflow-hidden bg-gradient-to-br from-blue-500 to-blue-600">
        {!imageError && (job?.thumbnail || job?.image || job?.featured_image) ? (
          ...
          <>
            <div className="w-full h-full bg-gradient-to-br from-blue-500 via-workflow-primary to-[var(--color-primary)] group-hover:scale-110 transition-transform duration-500" />
      ...
      <div className="p-8">
        ...
        <div className="flex flex-col md:flex-row items-start justify-between gap-6 mb-8">
          <div className="flex items-start gap-6 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-xl md:text-2xl font-[800] tracking-tight text-[var(--color-text-primary)] dark:text-white mb-2 group-hover:text-workflow-primary transition-colors">
                {job?.title}
              </h3>
              <p className="text-[var(--color-text-tertiary)] dark:text-slate-400 font-bold mb-4 flex items-center gap-2">
                {job?.company}
              ...
              <div className="flex flex-wrap gap-4 text-xs">
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-white/5 text-[var(--color-text-secondary)] dark:text-slate-400 font-bold">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div>
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-xs font-black uppercase tracking-widest text-[var(--color-text-tertiary)]">Matching Assets</h4>
            ...
            <div className="flex flex-wrap gap-2">
              {job?.matchedSkills?.slice(0, 6)?.map((skill, index) => (
                <span
                  key={index}
                  className="px-3 py-1.5 bg-[var(--color-primary)]/5 dark:bg-[var(--color-primary)]/10 text-[var(--color-primary)] dark:text-[var(--color-primary)] rounded-xl text-xs font-bold border border-[var(--color-primary)]/10 flex items-center gap-1.5"
                >
              ...
              {job?.matchedSkills?.length > 6 && (
                <span className="px-3 py-1.5 bg-slate-100 dark:bg-white/5 text-[var(--color-text-tertiary)] rounded-xl text-xs font-bold">
          ...
          {job?.skillGaps?.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-xs font-black uppercase tracking-widest text-[var(--color-text-tertiary)]">Skill Acquisition</h4>
        ...
        <div className="grid grid-cols-3 gap-1 p-1 bg-slate-100 dark:bg-white/5 rounded-2xl mb-8">
          <div className="flex flex-col items-center justify-center p-3 rounded-xl bg-white dark:bg-slate-900 shadow-sm">
            ...
            <p className="text-xs font-black text-[var(--color-text-primary)] dark:text-white capitalize">{job?.insights?.demandTrend || 'High'}</p>
          </div>
          <div className="flex flex-col items-center justify-center p-3 rounded-xl bg-white dark:bg-slate-900 shadow-sm">
            <div className="flex items-center gap-1.5 mb-1">
            ...
            <p className="text-xs font-black text-[var(--color-text-primary)] dark:text-white">{job?.applicationSuccess || '84'}%</p>
          </div>
          <div className="flex flex-col items-center justify-center p-3 rounded-xl bg-white dark:bg-slate-900 shadow-sm">
            <div className="flex items-center gap-1.5 mb-1">
            ...
            <p className="text-xs font-black text-[var(--color-text-primary)] dark:text-white">{job?.insights?.avgTimeToHire || '14 Days'}</p>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <Link
            ...
            className="flex-1 h-14 bg-slate-900 dark:bg-white text-white dark:text-[var(--color-text-primary)] rounded-2xl font-black text-sm flex items-center justify-center hover:scale-[1.02] shadow-xl transition-all"
          ...
          <div className="flex gap-2">
            <button
              className="w-14 h-14 rounded-2xl bg-white dark:bg-white/5 border border-[var(--color-border-primary)] dark:border-white/10 flex items-center justify-center text-slate-400 hover:text-workflow-primary hover:border-workflow-primary transition-all"
              onClick={(e) => { e?.stopPropagation(); }}
            >
