...
const LessonBlockBuilder = ({ blocks = [], onChange }) => {
    ...
    // Ensure blocks is an array (failsafe against old DB schema)
    ...
    const addBlock = (type) => {
        ...
        if (type === 'hadith') {
            newBlock.content = { english: '', arabic: '', narrator: '', reference: '' };
        } else if (type === 'quran') {
            newBlock.content = { surah: '', verse: '', arabic: '', translation: '' };
        } else if (type === 'scripture') {
            newBlock.content = { arabic: '', translation: '', reference: '' };
        } else if (type === 'design_markdown' || type === 'text') {
            newBlock.content = { text: '', title: '', footer: '' };
        } else if (type === 'video') {
            newBlock.url = '';
            ...
            newBlock.content = { title: '', duration: '', url: '' };
        } else if (type === 'document') {
            newBlock.url = '';
            newBlock.content = { fileName: '', fileSize: '', url: '' };
        } else if (type === 'summary') {
            newBlock.content = { points: ['', '', ''] };
        } else if (type === 'quiz') {
            newBlock.question = '';
            newBlock.options = ['', '', '', ''];
        ...
        onChange([...activeBlocks, newBlock]);
    };
    ...
    const updateBlock = (id, updates) => {
        onChange(activeBlocks.map(b => b.id === id ? { ...b, ...updates } : b));
    };

    const updateBlockContent = (id, contentUpdates) => {
        onChange(activeBlocks.map(b => 
            b.id === id 
    ...
    const removeBlock = (id) => {
        onChange(activeBlocks.filter(b => b.id !== id));
    };
    ...
    return (
        <div className="space-y-8">
            <div className="flex items-center justify-between border-b border-emerald-100 dark:border-emerald-500/10 pb-8">
                <div>
                    <h4 className="text-[11px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400">Lesson Content</h4>
                    <p className="text-sm text-slate-500 mt-2 font-medium">Design your learning experience with resizable, interactive blocks.</p>
                </div>
            </div>
