...
const ContentBlock = ({ 
  ...
  const renderPreview = () => {
    switch (block.type) {
      ...
      case 'video':
        return (
          <div className="w-full h-24 bg-slate-900 rounded-lg flex items-center justify-center border border-white/5">
            <VideoIcon className="w-6 h-6 text-[var(--color-primary)] opacity-50" />
            <span className="ml-2 text-xs text-white/70 italic line-clamp-1 px-4">{content.url || 'No URL'}</span>
          </div>
        );
      case 'text':
        return <div className="text-xs text-[var(--color-text-tertiary)] italic line-clamp-2">{content.text || content.content || 'Empty text block'}</div>;
      case 'quran':
      case 'hadith':
        return (
          <div className="flex flex-col gap-1">
            <div className="text-xs font-bold text-[var(--color-primary)] line-clamp-1">{content.translation || content.content || 'No translation'}</div>
            <div className="text-[10px] text-slate-400 font-arabic truncate text-right">{content.arabic || 'No Arabic script'}</div>
          </div>
        );
      case 'callout':
      case 'conclusion':
        return <div className="text-xs text-[var(--color-text-tertiary)] border-l-2 border-[var(--color-primary)] pl-2 italic line-clamp-1">{content.content || 'Empty synthesis'}</div>;
      case 'infographic':
      ...
      case 'quiz':
        return <div className="text-xs text-[var(--color-text-secondary)] font-bold"><HelpCircle size={12} className="inline mr-1"/> {content.question || 'No question'}</div>;
      default:
  ...
  return (
    <motion.div
      ...
      className={`bg-white dark:bg-slate-900 rounded-2xl border-2 mb-4 overflow-hidden shadow-sm transition-all ${
        isDragging
          ? 'border-[var(--color-primary)] shadow-xl opacity-50 scale-95'
          : 'border-slate-100 dark:border-white/5 hover:border-emerald-300 dark:hover:border-[var(--color-primary)]/30'
      }`}
    >
      <div className="p-4 md:p-5">
        <div className="flex items-center gap-4">
          <button onMouseDown={onDragStart} className="cursor-grab active:cursor-grabbing text-slate-300 hover:text-[var(--color-primary)] transition-colors">
            <GripVertical size={20} />
          </button>

          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-3">
              <div className="flex items-center gap-2 px-2.5 py-1 bg-[var(--color-primary-light)] dark:bg-[var(--color-primary)]/10 rounded-lg border border-[var(--color-border-secondary)]/50 dark:border-[var(--color-primary)]/10">
                {getBlockIcon()}
                <span className="text-[10px] font-black uppercase tracking-widest text-[var(--color-primary)] dark:text-[var(--color-primary)]">
                  {block.type}
                </span>
              </div>
          ...
          {!isEditing && (
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-2 text-slate-400 hover:text-[var(--color-primary)] hover:bg-[var(--color-primary-light)] dark:hover:bg-[var(--color-primary)]/10 rounded-xl transition-all"
              >
                <Edit2 size={18} />
              </button>
              <button
...
const ContentBlockEditor = ({ block, content, setContent, onSave, onCancel }) => {
  const inputClass = "w-full p-3 border border-[var(--color-border-primary)] dark:border-white/10 rounded-xl bg-white dark:bg-slate-950 text-[var(--color-text-primary)] dark:text-white text-sm focus:ring-2 focus:ring-[var(--color-primary)]/20 focus:border-[var(--color-primary)] outline-none transition-all";
      ...
      case 'infographic':
        return (
          <div className="space-y-6">
            <div className="flex gap-4 p-4 bg-white dark:bg-black/20 rounded-2xl border border-slate-100 dark:border-white/5">
              <button onClick={() => setContent({...content, layout: 'grid'})} className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${content.layout === 'grid' ? 'bg-[var(--color-primary)] text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-400'}`}>Grid Layout</button>
              <button onClick={() => setContent({...content, layout: 'process'})} className={`flex-1 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${content.layout === 'process' ? 'bg-[var(--color-primary)] text-white shadow-lg' : 'bg-white dark:bg-slate-800 text-slate-400'}`}>Process Flow</button>
            </div>
            <div className="space-y-4">
              ...
              <button 
                ...
                className="w-full py-3 border-2 border-dashed border-[var(--color-primary)]/30 text-[var(--color-primary)] rounded-2xl hover:bg-[var(--color-primary-light)] transition-all font-bold text-xs"
            ...
            <div className="space-y-2">
              {(content.options || ['', '', '', '']).map((opt, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  <button onClick={() => setContent({...content, correctIndex: i})} className={`px-4 rounded-xl text-[10px] font-black tracking-widest transition-all ${content.correctIndex === i ? 'bg-[var(--color-primary)] text-white shadow-lg' : 'bg-slate-100 text-slate-400'}`}>CORRECT</button>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/5 space-y-6">
      ...
      <div className="flex gap-3 justify-end pt-4">
        <button onClick={onCancel} className="px-5 py-2 text-xs font-black uppercase tracking-widest text-[var(--color-text-tertiary)] hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all">Cancel</button>
        <button onClick={onSave} className="px-6 py-2 text-xs font-black uppercase tracking-widest text-white bg-[var(--color-primary)] hover:bg-[var(--color-primary-hover)] rounded-xl transition-all shadow-lg flex items-center gap-2"><Save size={14} /> Save Block</button>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="relative mt-8">
      <button
        ...
        className="w-full py-5 px-6 border-2 border-dashed border-[var(--color-primary)]/20 dark:border-[var(--color-primary)]/10 text-[var(--color-primary)] dark:text-[var(--color-primary)] rounded-3xl hover:bg-[var(--color-primary-light)]/50 dark:hover:bg-[var(--color-primary)]/5 transition-all flex items-center justify-center gap-3 font-black text-xs uppercase tracking-[0.2em] shadow-sm hover:shadow-md"
      >
              ...
              {blockTypes.map((bt) => {
                ...
                return (
                  <button
                    ...
                    className="p-4 text-left hover:bg-[var(--color-primary-light)] dark:hover:bg-[var(--color-primary)]/10 rounded-2xl transition-all flex items-start gap-3 group"
                  >
                    <div className="w-10 h-10 rounded-xl bg-white dark:bg-white/5 flex items-center justify-center shrink-0 group-hover:bg-white dark:group-hover:bg-slate-800 transition-colors shadow-sm">
                      <Icon className="w-5 h-5 text-[var(--color-primary)] dark:text-[var(--color-primary)]" />
                    </div>
                    <div>
                      <p className="font-black text-slate-800 dark:text-white text-[10px] uppercase tracking-widest mb-1 group-hover:text-[var(--color-primary)] transition-colors">
                        {bt.label}
                      </p>
