...
const AIChatPanel = ({ onUpdateResume, currentResume }) => {
    ...
    const QuickAction = ({ icon: Icon, label, color, tool }) => (
        <button
            ...
            className="flex flex-col items-center justify-center gap-1.5 p-2 rounded-lg bg-dark-surface-elevated border border-dark-border hover:bg-dark-border hover:border-workflow-primary hover:shadow-lg transition-all group relative overflow-hidden"
        >
            <div className={`p-1.5 rounded-md bg-opacity-10 ${color} group-hover:bg-opacity-20 transition-colors`}>
                <Icon className={`w-4 h-4 ${color.replace('bg-', 'text-')}`} />
            </div>
            <span className="text-[9px] font-bold uppercase tracking-wider text-dark-text-secondary group-hover:text-white relative z-10 text-center leading-tight">{label}</span>
        </button>
    );

    return (
        <div className="flex flex-col h-full bg-dark-surface border-r border-l border-dark-border">

            {/* 1. Header & Trigger */}
            <div className="flex items-center justify-between p-3 border-b border-dark-border bg-dark-bg select-none">
                <div className="flex items-center gap-2">
                    <Bot className="w-4 h-4 text-emerald-500" />
                    <span className="text-sm font-bold text-dark-text tracking-wide">AI Copilot</span>
                </div>
                <button
                    onClick={() => setShowTools(!showTools)}
                    className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-workflow-primary-300 hover:text-workflow-primary-200 bg-workflow-primary/10 px-2 py-1 rounded-full transition-colors"
                >
            ...
            <AnimatePresence>
                {showTools && (
                    <motion.div
                        ...
                        className="overflow-hidden bg-dark-bg border-b border-dark-border"
                    >
                        <div className="p-3 grid grid-cols-4 gap-2">
                            <QuickAction icon={Wand2} label="Fix Fmt" tool="Format Fixer" color="bg-workflow-primary" />
                            <QuickAction icon={Target} label="JD Match" tool="Job Matcher" color="bg-emerald-500" />
                            <QuickAction icon={Star} label="Skill Gap" tool="Skill Gap" color="bg-warning-500" />
                            <QuickAction icon={TrendingUp} label="Metrics" tool="Metric Master" color="bg-workflow-primary-400" />

                            <QuickAction icon={Search} label="Debuzz" tool="Buzzword Blast" color="bg-error-500" />
            ...
            {/* 3. Chat Stream */}
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-4 bg-dark-surface" ref={scrollRef}>
                {messages.map((msg, i) => (
                    <motion.div
                    ...
                    >
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${msg.role === 'user' ? 'bg-workflow-primary' : 'bg-emerald-600'}`}>
                        ...
                        <div className={`flex flex-col gap-2 max-w-[85%]`}>
                            <div className={`p-3 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-md ${msg.role === 'user'
                                ...
                                : 'bg-dark-surface-elevated text-dark-text border border-dark-border rounded-tl-none'
                                }`}>
                                {msg.text}
                            </div>
                            ...
                            {msg.pendingData && !msg.applied && !msg.discarded && (
                                <motion.div
                                    ...
                                    className="bg-dark-bg border border-workflow-primary/30 p-3 rounded-xl shadow-lg mt-1"
                                >
                                    <div className="flex items-center gap-2 text-workflow-primary-300 text-xs font-bold mb-2 uppercase tracking-wide">
                                        <Sparkles className="w-3 h-3" /> Suggested Changes
                                    </div>
                                    <div className="flex gap-2">
                                        <button
                                            onClick={() => handleApplyChanges(i, msg.pendingData)}
                                            className="flex-1 bg-workflow-primary hover:bg-workflow-primary-600 text-white text-xs font-bold py-1.5 px-3 rounded-lg flex items-center justify-center gap-1 transition-colors"
                                        >
                                            <Check className="w-3 h-3" /> Apply
                                        </button>
                                        <button
                                            onClick={() => handleDiscardChanges(i)}
                                            className="flex-1 bg-dark-surface hover:bg-dark-border text-dark-text-secondary text-xs font-bold py-1.5 px-3 rounded-lg flex items-center justify-center gap-1 transition-colors border border-dark-border"
                                        >
                            ...
                            {msg.applied && (
                                <div className="text-[10px] text-success-500 font-bold flex items-center gap-1 ml-1">
                                    <CheckCircle2 className="w-3 h-3" /> Changes Applied
                                </div>
                            )}
                            {msg.discarded && (
                                <div className="text-[10px] text-dark-text-muted font-bold flex items-center gap-1 ml-1">
                ...
                {thinking && (
                    <div className="flex gap-3">
                        ...
                        <div className="p-3 bg-dark-surface-elevated border border-dark-border rounded-2xl rounded-tl-none">
                            <div className="flex gap-1 pt-1">
                                <div className="w-1.5 h-1.5 rounded-full bg-dark-text-muted animate-bounce" style={{ animationDelay: '0ms' }} />
                                <div className="w-1.5 h-1.5 rounded-full bg-dark-text-muted animate-bounce" style={{ animationDelay: '150ms' }} />
                                <div className="w-1.5 h-1.5 rounded-full bg-dark-text-muted animate-bounce" style={{ animationDelay: '300ms' }} />
                            </div>
                        </div>
                    </div>
                )}
            ...
            {/* 4. Input Area */}
            <div className="p-3 bg-dark-bg border-t border-dark-border">
                <div className="relative">
                    <textarea
                        ...
                        className="w-full pl-3 pr-10 py-3 bg-dark-surface border border-dark-border rounded-xl focus:ring-1 focus:ring-workflow-primary outline-none text-xs sm:text-sm min-h-[45px] max-h-[100px] resize-none text-white placeholder-dark-text-muted"
                    />
                    <button
                        onClick={handleSend}
                        disabled={!input.trim() || thinking}
                        className="absolute right-2 bottom-2 p-1.5 bg-workflow-primary hover:bg-workflow-primary-600 disabled:bg-dark-border disabled:opacity-50 text-white rounded-lg transition-all"
                    >
                        <Send className="w-4 h-4" />
                    </button>
                </div>
                <div className="text-[9px] text-center mt-2 text-dark-text-muted font-medium">
