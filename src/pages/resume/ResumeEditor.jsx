...
const ResumeEditor = () => {
    ...
    return (
        <div className="h-screen flex flex-col bg-slate-50 dark:bg-slate-900 overflow-hidden font-sans">
            ...
            {/* Resizable Panels Floating Layout (Desktop) vs Stacked (Mobile) */}
            <div className="flex-1 overflow-hidden px-4 pb-4">
                {/* Mobile View (< md) */}
                <div className="md:hidden h-full relative text-sm">
                    {showPreview ? (
                        <div className="h-full overflow-y-auto rounded-2xl bg-slate-100 dark:bg-[#0B0F17] border border-slate-200 dark:border-slate-800 shadow-inner p-4">
                            <ResumePreview data={resumeData} template={resume?.template_id} />
                            {/* Floating Edit Button for Mobile */}
                            <button
                                onClick={() => setShowPreview(false)}
                                className="fixed bottom-6 right-6 p-4 bg-indigo-600 text-white rounded-full shadow-xl hover:bg-indigo-700 transition-all z-50 flex items-center gap-2"
                            >
                        ...
                        <div className="h-full flex flex-col rounded-2xl bg-white/50 dark:bg-slate-800/50 backdrop-blur-md border border-white/20 dark:border-slate-700 shadow-xl overflow-hidden">
                            <div className="flex-1 overflow-y-auto custom-scrollbar">
                                <TemplateSelector currentTemplate={resume?.template_id} onSelect={handleTemplateChange} />
                            ...
                            >
                                ...
                                <span className="font-bold">Preview PDF</span>
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className="hidden md:block h-full">
                    <PanelGroup direction="horizontal" className="h-full gap-4">
                        {/* Editor (Left Pane) - 40% Width */}
                        <Panel defaultSize={40} minSize={30} className="flex flex-col rounded-2xl bg-white/50 dark:bg-slate-800/50 backdrop-blur-md border border-white/20 dark:border-slate-700 shadow-xl overflow-hidden">
