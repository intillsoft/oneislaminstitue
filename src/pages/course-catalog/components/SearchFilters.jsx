    ...
    <div className="bg-white dark:bg-[#0f1429] p-5 rounded-[2rem] space-y-7 border border-slate-100 dark:border-white/5 shadow-sm overflow-hidden">
      <div className="flex items-center justify-between pb-3 border-b border-slate-50 dark:border-white/5">
        ...
        {activeFiltersCount > 0 && (
          <button
            onClick={onClearAll}
            className="text-[9px] text-workflow-primary hover:text-[var(--color-primary)] font-black uppercase tracking-widest transition-all"
          >
            ...
            {visibleCategories.map((category) => (
              <label key={category} className="flex items-center gap-2.5 cursor-pointer group">
                <input
                  ...
                  className="w-3.5 h-3.5 rounded-md border-[var(--color-border-primary)] dark:border-white/10 bg-white dark:bg-slate-800 text-workflow-primary focus:ring-[var(--color-primary)]/20 transition-all cursor-pointer"
                />
                <span className="text-[11px] font-medium text-[var(--color-text-tertiary)] dark:text-slate-400 group-hover:text-[var(--color-primary)] transition-colors">{category}</span>
              </label>
            ))}
            <button 
            ...
            {studyModes.map((mode) => (
              <label key={mode} className="flex items-center gap-2.5 cursor-pointer group">
                <input
                  ...
                  className="w-3.5 h-3.5 rounded-md border-[var(--color-border-primary)] dark:border-white/10 bg-white dark:bg-slate-800 text-workflow-primary focus:ring-[var(--color-primary)]/20 transition-all cursor-pointer"
                />
                <span className="text-[11px] font-medium text-[var(--color-text-tertiary)] dark:text-slate-400 group-hover:text-[var(--color-primary)] transition-colors">{mode}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
           <div className="space-y-2">
              ...
              <select
                ...
                className="w-full bg-white dark:bg-slate-800/50 border border-slate-100 dark:border-white/5 rounded-xl px-3 py-2 text-[11px] font-bold text-[var(--color-text-secondary)] dark:text-slate-300 outline-none focus:border-[var(--color-primary)]/30 cursor-pointer transition-all appearance-none"
              >
      ...
      {onClose && (
        <button
          onClick={onClose}
          className="w-full bg-[var(--color-primary)] text-white py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg shadow-[var(--color-primary)]/20 active:scale-95 transition-all"
        >
          Update Search
        </button>
      )}
