    ...
    <div className="space-y-10">
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6 px-4">
        <div>
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-text-tertiary)] mt-1">Platform Asset Integrity & Verification Hub</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <EliteCard className="p-0 border-white/5 bg-white/[0.01] overflow-hidden">
            <div className="p-6 bg-white/[0.02] border-b border-white/5 flex items-center justify-between">
              <h3 className="text-[10px] font-black text-[var(--color-text-tertiary)] uppercase tracking-widest">PENDING ASSETS</h3>
            ...
            <div className="divide-y divide-white/[0.03] max-h-[600px] overflow-y-auto">
              {jobsRequiringApproval.map((job) => (
                <button key={job.id} onClick={() => setSelectedJob(job)} className={`w-full p-6 text-left transition-all hover:bg-white/[0.02] ${selectedJob?.id === job.id ? 'bg-blue-500/10 border-l-2 border-l-blue-500' : ''}`}>
                  ...
                  <p className="text-[9px] font-bold text-[var(--color-text-tertiary)] uppercase tracking-widest mt-1">{job.company}</p>
          ...
          {selectedJob ? (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <EliteCard className="p-8 border-white/5 bg-white/[0.01]">
                <div className="flex items-start justify-between mb-8">
                  <div>
                    ...
                    <p className="text-[10px] font-black text-[var(--color-text-tertiary)] uppercase tracking-widest mt-1">{selectedJob.company} • {selectedJob.location}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest">{selectedJob.salary}</p>
                    <p className="text-[9px] font-bold text-[var(--color-text-secondary)] uppercase tracking-widest mt-1">Origin: {selectedJob.postedBy}</p>
                  </div>
                </div>

                <div className="space-y-8 border-t border-white/5 pt-8">
                  <div>
                    <h4 className="text-[9px] font-black text-[var(--color-text-secondary)] uppercase tracking-widest mb-4">Forensic Analysis: Description</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                      <h4 className="text-[9px] font-black text-[var(--color-text-secondary)] uppercase tracking-widest mb-4">Parameters: Requirements</h4>
                      <ul className="space-y-2">
                        {selectedJob.originalContent.requirements.map((req, i) => (
                          <li key={i} className="text-[10px] font-bold text-[var(--color-text-tertiary)] uppercase tracking-wide flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="text-[9px] font-black text-[var(--color-text-secondary)] uppercase tracking-widest mb-4">Metadata: Benefits</h4>
                      <ul className="space-y-2">
                        {selectedJob.originalContent.benefits.map((benefit, i) => (
                          <li key={i} className="text-[10px] font-bold text-[var(--color-text-tertiary)] uppercase tracking-wide flex items-center gap-3">
                            <span className="w-1 h-1 rounded-full bg-[var(--color-primary)]/40" /> {benefit}
                          </li>
                        ))}
                      </ul>
                    </div>
              ...
              <div className="flex items-center gap-4">
                <button onClick={() => handleApprove(selectedJob.id)} className="flex-1 py-4 bg-[var(--color-primary)] rounded-xl text-[10px] font-black uppercase tracking-widest text-white hover:bg-[var(--color-primary-hover)] transition-all shadow-lg shadow-[var(--color-primary)]/20 flex items-center justify-center gap-3">
            ...
            <EliteCard className="p-20 border-white/5 bg-white/[0.01] text-center">
              ...
              <h3 className="text-sm font-black text-[var(--color-text-secondary)] uppercase tracking-[0.2em]">Asset Selection Pending</h3>
