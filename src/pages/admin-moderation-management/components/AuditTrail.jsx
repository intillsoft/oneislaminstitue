...
const AuditTrail = () => {
  ...
  const getActionConfig = (action) => {
    const config = {
      ...
      job_approved: { color: 'text-[var(--color-primary)]', icon: 'CheckCircle' },
      content_rejected: { color: 'text-rose-500', icon: 'XCircle' },
      settings_updated: { color: 'text-blue-500', icon: 'Settings' },
    };
    return config[action] || { color: 'text-[var(--color-text-tertiary)]', icon: 'Activity' };
  };
    ...
    <div className="space-y-10">
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-8 px-4">
        <div>
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-text-tertiary)] mt-1">Platform Activity & Forensic Governance</p>
        </div>

        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group">
            <Icon name="Search" size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-text-secondary)] group-hover:text-blue-500 transition-colors" />
            <input
              type="text"
              value={searchTerm}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
        <div className="xl:col-span-8 space-y-6">
          <EliteCard className="p-10 border-white/5 bg-white/[0.01]">
            <h3 className="text-[10px] font-black text-[var(--color-text-tertiary)] uppercase tracking-[0.3em] mb-12">Forensic Timeline</h3>
            ...
            {loading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-[10px] font-black text-[var(--color-text-secondary)] uppercase tracking-widest">No activities detected in current scope</p>
              </div>
            ) : (
                      ...
                      <div className="group relative">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-4">
                            ...
                            <span className="text-[10px] font-black text-[var(--color-text-secondary)] uppercase tracking-widest font-mono">{activity.timestamp}</span>
                          </div>
                          <button className="text-[9px] font-black text-[var(--color-text-tertiary)] uppercase tracking-widest hover:text-white transition-colors">Trace ID: {activity.id.slice(0, 8)}</button>
                        </div>
                        ...
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] font-bold text-[var(--color-text-secondary)] uppercase tracking-widest">Access Level: {activity.user?.role}</p>
                          </div>
                        </div>

                        <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 space-y-3">
                          {activity.target && <p className="text-[10px] font-black text-[var(--color-text-tertiary)] uppercase tracking-widest">Target Asset: <span className="text-white">{activity.target}</span></p>}
        ...
        <div className="xl:col-span-4 space-y-6">
          ...
          <EliteCard className="p-8 border-white/5 bg-white/[0.01]">
            <h3 className="text-[10px] font-black text-[var(--color-text-tertiary)] uppercase tracking-[0.2em] mb-8">Governance Matrix</h3>
            <div className="space-y-8">
              {[
                { label: 'Security Protocols', value: 45, color: 'bg-rose-500' },
                { label: 'Asset Sync', value: 30, color: 'bg-blue-500' },
                { label: 'Registry Update', value: 25, color: 'bg-[var(--color-primary)]' },
              ].map((item) => (
