    ...
    <div className="space-y-10">
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6 px-4">
        <div>
          ...
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[var(--color-text-tertiary)] mt-1">Curator Team Accreditation & Clearance Registry</p>
        </div>
      </div>
      ...
      <EliteCard className="p-2 border-white/5 bg-white/[0.02] rounded-[1.5rem] overflow-hidden">
        <div className="flex flex-wrap gap-2">
          {['all', 'pending', 'approved', 'rejected'].map((status) => (
            <button key={status} onClick={() => setFilter(status)} className={`px-8 py-4 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === status ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-[var(--color-text-tertiary)] hover:text-white hover:bg-white/5'}`}>
              {status}
            </button>
          ))}
        </div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/[0.02] border-b border-white/5">
                <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-[var(--color-text-secondary)]">IDENTIFIER</th>
                <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-[var(--color-text-secondary)]">TARGET CLEARANCE</th>
                <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-[var(--color-text-secondary)]">STATE</th>
                <th className="px-8 py-5 text-[9px] font-black uppercase tracking-widest text-[var(--color-text-secondary)]">TIMESTAMP</th>
                <th className="px-8 py-5 text-right text-[9px] font-black uppercase tracking-widest text-[var(--color-text-secondary)]">HUB</th>
              </tr>
            </thead>
              ...
              {requests.length > 0 ? (
                  ...
                  <tr key={request.id} className="group hover:bg-white/[0.01] transition-all">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4 text-white">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] font-medium text-[var(--color-text-tertiary)]">{request.user?.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6">
                      <span className={`text-[9px] font-black uppercase tracking-widest ${request.status === 'pending' ? 'text-amber-500' :
                        request.status === 'approved' ? 'text-[var(--color-primary)]' : 'text-rose-500'
                        }`}>
                        {request.status}
                      </span>
                    </td>
                    <td className="px-8 py-6 text-[10px] font-black text-[var(--color-text-secondary)] uppercase">
                      {formatDistanceToNow(new Date(request.created_at), { addSuffix: true })}
                    </td>
                    <td className="px-8 py-6 text-right">
                      {request.status === 'pending' && (
                        <div className="flex items-center justify-end gap-3">
                          <button onClick={() => handleApprove(request.id)} className="px-4 py-2 bg-[var(--color-primary)]/10 border border-emerald-600/20 text-[var(--color-primary)] text-[9px] font-black uppercase tracking-widest rounded-lg hover:bg-[var(--color-primary)] hover:text-white transition-all">AUTHORIZE</button>
                ...
                <tr>
                  <td colSpan="5" className="px-8 py-20 text-center">
                    ...
                    <p className="text-[10px] font-black text-[var(--color-text-secondary)] uppercase tracking-[0.2em]">Registry Clear</p>
                  </td>
                </tr>
              )}
            </tbody>
