...
const StatCard = ({ icon: iconName, label, value, color }) => {
  const configs = {
    primary:   { border: '#0078D4', icon: '#0078D4', bg: 'var(--secondary)' },
    secondary: { border: '#4BA8E8', icon: '#4BA8E8', bg: 'var(--secondary)' },
    accent:    { border: 'var(--primary)', icon: 'var(--primary)', bg: 'var(--secondary)' },
  ...
  return (
    <motion.div
      ...
      className="relative h-36 overflow-hidden rounded-xl bg-[var(--card)] border border-[var(--border)] p-6 flex flex-col justify-between"
      style={{ borderTopWidth: '4px', borderTopColor: cfg.border, boxShadow: 'var(--shadow-card)' }}
    >
      <div className="flex items-start justify-between">
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-bold text-[var(--muted-foreground)] uppercase tracking-[0.15em] mb-3">{label}</p>
          <p className="text-3xl font-extrabold text-[var(--foreground)] tracking-tight tabular-nums">
            {value}
          </p>
        </div>
