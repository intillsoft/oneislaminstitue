...
const DemographicInsights = () => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-[var(--card)] backdrop-blur-xl p-4 border border-[var(--border)] rounded-2xl shadow-2xl">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--muted-foreground)] mb-2">{label}</p>
          <div className="flex items-center justify-between gap-8">
            <span className="text-xs font-black text-[var(--foreground)]">{payload[0].value}%</span>
            <span className="text-[10px] font-bold text-[var(--muted-foreground)]">({payload[0].payload.count} users)</span>
          </div>
        </div>
      );
    }
  ...
  return (
    <EliteCard className="p-10 overflow-hidden relative" hover={false}>
      <div className="absolute top-0 left-0 w-full h-full opacity-[0.02] pointer-events-none">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,var(--foreground)_1px,transparent_1px)] bg-[length:20px_20px]" />
      </div>

      <div className="relative z-10">
        <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-8 mb-12">
          <div>
            <h3 className="text-xs font-black text-[var(--muted-foreground)] uppercase tracking-[0.2em] mb-1">Human Capital Meta-Data</h3>
            <h2 className="text-2xl font-black text-[var(--foreground)] uppercase tracking-tight">Candidate Demographics</h2>
          </div>
          <div className="flex flex-wrap p-1.5 bg-[var(--secondary)] rounded-2xl border border-[var(--border)] backdrop-blur-md">
        ...
        {data.length === 0 ? (
          <div className="h-72 flex items-center justify-center text-[var(--muted-foreground)] border border-dashed border-[var(--border)] rounded-3xl">
        ...
        <div className="mt-8 flex items-center justify-between border-t border-[var(--border)] pt-6">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[var(--primary)]/10 flex items-center justify-center">
              <Icon name="ShieldCheck" className="w-4 h-4 text-[var(--primary)]" />
            </div>
            <p className="text-[10px] font-bold text-[var(--muted-foreground)] uppercase tracking-widest leading-tight">
              Anonymized data stream protected by <br /><span className="text-[var(--foreground)]">Workflow Encryption Standards</span>
            </p>
          </div>
          <div className="hidden sm:flex flex-wrap gap-4">
            {data.map((item, idx) => (
              <div key={idx} className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-black text-[var(--muted-foreground)] uppercase tracking-widest">{item.name}</span>
              </div>
            ))}
          </div>
        </div>
