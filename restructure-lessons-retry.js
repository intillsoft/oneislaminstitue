...
async function updateLessonWithRetry(lessonId, finalBlocks, title, type, maxRetries = 5) {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.log(`[RETRY ${attempt}/${maxRetries}] Failed on ${title}: ${err.message}`);
            await sleep(2000 * attempt);
        }
    }
    console.log(`[FATAL] Failed to update ${title} after ${maxRetries} attempts.`);
    return false;
}

async function run() {
    console.log('--- RESTRUCTURING LESSON BLOCKS (WITH RETRY) ---');
    ...
    // Handle transient fetch errors even on the SELECT
    if (lessErr) {
        console.log('Initial Fetch Error:', lessErr);
        await sleep(3000);
        let retryRes = await supabase.from('course_lessons').select('*').eq('course_id', COURSE_ID);
        lessons = retryRes.data;
        if (retryRes.error) return console.log('Fatal Fetch Error:', retryRes.error);
    }
    
    if (!lessons) return console.log('No lessons found to restructure.');
    ...
    for (let i = 0; i < lessons.length; i++) {
        ...
        if (isAssessment) {
            ...
            if (quizBlocks.length > 10) quizBlocks = quizBlocks.slice(0, 10);
            newBlocks = quizBlocks;
        } else {
            // Standard: Non-quiz first, quiz last
        ...
        const success = await updateLessonWithRetry(lesson.id, finalBlocks, lesson.title, isAssessment ? 'Assessment 10 Qs' : 'Standard');
        if (success) updatedCount++;
        
        await sleep(150); // Be nice to the API
    }
    
    console.log(`\n--- Completed. Successfully Restructured ${updatedCount}/${lessons.length} lessons with enforced block orders! ---`);
}
