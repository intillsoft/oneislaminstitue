    ...
    pages: [
        {
          ...
          "page_type": "overview",
          "content": [
            { "id": genId(), "type": "image", "content": { "url": "https://image.pollinations.ai/prompt/realistic%20silhouette%20person%20stars%20spark%20horizontal?width=1200&height=600" } },
        ...
        },
        {
          ...
          "content": [
            { "id": genId(), "type": "quiz", "content": { "question": "What is fitrah?", "options": ["The soul after death", "Innate human nature inclined to believe in God", "A type of prayer"], "correctIndex": 1 } }
          ]
        }
    ]
  },
...
async function start() {
    ...
    for (let i = 0; i < lessonsData.length; i++) {
         ...
         const dbL = dbLessons[i]; // 0 -> 1.1, 1 -> 1.2
         await supabase.from('course_lessons').update({ title: l.title, content_data: { page_count: l.pages.length, is_time_gated: false, pages: l.pages } }).eq('id', dbL.id);
         console.log(`✅ Loaded ${l.title}`);
    }
}
