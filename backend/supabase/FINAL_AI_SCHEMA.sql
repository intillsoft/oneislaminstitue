...
-- RLS Policies for AI Tables - ALL FIXED
...
DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename IN ('embeddings_cache', 'interview_questions', 'interview_sessions', 'salary_cache', 'salary_reports', 'career_analyses')) LOOP
        EXECUTE 'DROP POLICY IF EXISTS "Service role can manage embeddings" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Authenticated users can manage embeddings" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Anyone can read interview questions" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Authenticated users can manage questions" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Users can manage own interview sessions" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Service role can manage salary cache" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Authenticated users can manage salary cache" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Users can create salary reports" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Authenticated users can read salary reports" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "Users can manage own career analyses" ON ' || quote_ident(r.tablename);
    END LOOP;
END $$;

-- Embeddings Cache: Allow authenticated users
...
-- Salary Cache: Allow authenticated users
