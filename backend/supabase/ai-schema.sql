-- Additional Schema for AI Services
-- 
-- IMPORTANT: Before running this schema, enable the pgvector extension:
...
-- Embeddings Cache: Service role only (allow all for now, restrict via backend)
CREATE POLICY "Service role can manage embeddings"
    ON embeddings_cache FOR ALL
    USING (true); -- Allow all, restrict via backend authentication
...
-- Salary Cache: Service role only (allow all for now, restrict via backend)
