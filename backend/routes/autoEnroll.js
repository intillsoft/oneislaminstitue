...
/**
 * POST /api/auto-apply/logs/:id/confirm
 * Confirm manual application of a job
 */
router.post('/logs/:id/confirm', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await autoApplyService.confirmManualApply(req.user.id, id);
    res.json({ success: true, data: result });
  } catch (error) {
...
/**
 * POST /api/auto-apply/logs/:id/retry
 * Retry application using AI Agent
 */
router.post('/logs/:id/retry', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await autoApplyService.retryAutoApply(req.user.id, id);
    res.json({ success: true, data: result });
  } catch (error) {
