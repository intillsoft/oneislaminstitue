-- ============================================================================
-- FINAL DATABASE REPAIR SCRIPT (v3)
-- Resolves Enrollment Visibility (RLS), Notifications Crash, and Saved Courses
-- Uses the correct course schema variables (instructor_id) AND the correct saved_courses column (course_id)
-- ============================================================================
...
-- We know the column is already named `course_id`. Let's ensure it has a foreign key to `jobs`.
...
ALTER TABLE public.saved_courses ADD CONSTRAINT saved_courses_course_id_fkey FOREIGN KEY (course_id) REFERENCES public.jobs(id) ON DELETE CASCADE;
...
-- Note: In order to force Supabase's PostgREST to refresh its schema cache 
-- (which resolves PGRST200), we issue a schema cache reload command:
